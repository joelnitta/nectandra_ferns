...
The Nectandra Cloud Forest Reserve (hereafter, "Nectandra"; 10°11' N, 84°31' W) is located at \SI{1000}{\meter} to \SI{1200}{\meter} a.s.l. on the Atlantic slope of the Cordillera de Tilarán, Alajuela Province (`r figure("map")`a).
...
Herbarium codes follow @Thiers2019.
...
Our surveys resulted in `r taxa_count %>% pull(specimen)` individuals, representing `r taxa_count %>% pull(species)` species, `r taxa_count %>% pull(genus)` genera, and `r taxa_count %>% pull(family)` families of pteridophytes (`r s_table("checklist")`).
...
Our literature survey identified five other protected areas in mainland Costa Rica that have been surveyed for pteridophytes [@oet2019; @Haber1990; Kappelle2000; @Rojas2009; @Jimenez2016] (`r figure("map")`).
The site with the highest species richness is the Alberto Manuel Brenes Biological Reserve (281 \species), with Nectandra the second-highest and nearly the same as La Selva Biological Station (`r figure("richness-cr")`a).
Nectandra has by far the most species per hectare (`r nectandra_sp_per_hec`), with the San Luis Biological Reserve second-highest (`r figure("richness-cr")`b).
...
We recovered *rbcL* sequences for `r n_taxa_rbcL_seqs` taxa (`r coverage_taxa_rbcL_seqs`) from Nectandra, resulting in `r new_seqs_data$genomic_id %>% n_distinct` new *rbcL* sequences (`r s_table("checklist")`).
...
This failure rate is higher than that of the pteridophytes of Moorea and Tahiti, but lower than Japan (`r figure("min-dist")`).
...
When only sexual diploids are included, the failure rate in Japan drops and the distribution more closely resembles that of Costa Rica (`r figure("min-dist")`).
...
**`r s_table_full("genbank")`**

**`r s_table_full("checklist")`** All voucher specimens deposited in UC.
