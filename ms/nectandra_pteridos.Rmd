...
Despite its small size (`r si(51100, "km^2^")`), Costa Rica is home to a remarkably high biodiversity, and is ranked as one of the world's top 25 biodiversity hotspots [@Myers2000].
...
The genera with the most species were *`r top_genera %>% slice(1) %>% pull(genus)`* (`r top_genera %>% slice(1) %>% pull(n) %>% si("spp.")`), *`r top_genera %>% slice(2) %>% pull(genus)`* (`r top_genera %>% slice(2) %>% pull(n) %>% si("spp.")`), *`r top_genera %>% slice(3) %>% pull(genus)`* (`r top_genera %>% slice(3) %>% pull(n) %>% si("spp.")`), and *`r top_genera %>% slice(4) %>% pull(genus)`* (`r top_genera %>% slice(4) %>% pull(n) %>% si("spp.")`).
Families with the most species were `r top_families %>% slice(1) %>% pull(family)` (`r top_families %>% slice(1) %>% pull(n) %>% si("spp.")`), `r top_families %>% slice(2) %>% pull(family)` (`r top_families %>% slice(2) %>% pull(n) %>% si("spp.")`), and `r top_families %>% slice(3) %>% pull(family)` (`r top_families %>% slice(3) %>% pull(n) %>% si("spp.")`).
...
The site with the highest species richness is the Alberto Manuel Brenes Biological Reserve (`r cr_richness %>% arrange(desc(richness)) %>% slice(1) %>% pull(richness) %>% si("spp.")`), with Nectandra the second-highest and nearly the same as La Selva Biological Station (`r figure("richness-cr")`a).
...
Mean sequence length was `r mean(seq_lengths) %>% si("bp", accuracy = 1)` ± SD `r sd(seq_lengths) %>% si("bp", accuracy = 1)`.
*rbcL* could not be sequenced for `r (taxa_count$taxon - n_taxa_rbcL_seqs) %>% english2` taxa (of these, one already had a sequence available on GenBank), and only partial sequences (ca. 3' or 5' half of *rbcL*) could be obtained for `r seq_lengths[seq_lengths < 1000] %>% length() %>% english2` taxa.
...
One exception was Osmundaceae sister to Gleicheniaceae (SH-aLRT <50%; UFboot 95%), which may be due to poor sampling; we could obtain only a `r si(614, "bp")` *rbcL* fragment from the sole osmundaceous species at our study site.
