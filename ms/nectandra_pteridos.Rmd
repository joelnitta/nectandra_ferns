...
*rbcL* sequences were downloaded from GenBank for three taxa that could not be successfully sequenced in our study: *Pteris altissima* Poir. (*Nitta 863*, KM008147) and *Amauropelta atrovirens* (C. Chr.) Salino & T.E. Almeida (*Nitta 2237*, GenBank_XXX) from Nectandra, and *Trichomanes polypodioides* L. from Bolivia (*M. Kessler 8808*, AY175795).
...
We generated `r n_acc_rbcL_new` new *rbcL* sequences of pteridophytes from Nectandra representing `r n_taxa_rbcL_new` taxa (`r s_table("genbank")`).
...
<!-- Consider adding text: The "edge-effect" (CITE) may also contribute to the observed high richness: ca. _ taxa
only occur on the exposed edges of the reserve, and not in the interior. Mixing of secondary and primary forest may also increase richness (CITE); however, it seems that elevation plays a greater role, given the small number of "edge-effect" taxa. -->
...
**`r s_figure_full("cyatheaceae-tree")`** Tree rooted on Dicksoniaceae species from Nectandra.
...
**`r s_figure_full("grammitid-tree")`** Tree rooted on subfamily Polypodioideae species from Nectandra.
Numbers at nodes indicate local support values computed with the Shimodaira–Hasegawa test; values less than 50% not shown.
For phylogram on right side, scale bar shows expected number of changes per site.
Numbers after species names are GenBank accession numbers for sequences downloaded from GenBank or J. H. Nitta specimen collection numbers for sequences newly obtained by this study.
