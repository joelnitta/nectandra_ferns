...
We generally sampled one species per taxon from Nectandra for morphologically distinct taxa, and up to five specimens per taxon for taxa that are more difficult to identify using standard keys [@Moran1995; @Lellinger1989].
...
We amplified the plastid *rbcL* gene using PCR primers and thermocycler settings of @Schuettpelz2007 and verified amplification success by gel electrophoresis in 1% TAE.
...
We assessed the utility of *rbcL* for species identification by calculating minimum interspecific distances.
We first calculated all raw interspecific distances from the *rbcL* alignment using the "dist.dna" function in the APE package [@Paradis2004], then extracted the minimum interspecific distance for each species using custom scripts.
...
Our surveys resulted in `r taxa_count %>% pull(specimen)` individuals representing `r taxa_count %>% pull(species) %>% si("spp.")`, `r taxa_count %>% pull(genus)` genera, and `r taxa_count %>% pull(family)` families of pteridophytes from Nectandra (`r s_table("checklist")`).
...
Nectandra has the third-highest species richness of protected areas in Costa Rica with data available for ferns and lycophytes, and by far the greatest number of species per hectare (`r figure("richness-cr")`).
...
dem.raster <- getData("alt", country = "CR", path = tempdir())
...
costa_rica_map <- ggplot() +
  ...
  geom_point(
    data = cr_richness %>% filter(name != "Nectandra"), 
